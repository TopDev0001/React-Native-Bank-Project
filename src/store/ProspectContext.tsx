  ...
  const validateSigner = async (payload: ISignerDetails) => {
    ...
    if (!accountRequestId && !signerId && prospect.status === 'signerReady') {
      ...
      try {
        ...
        log.error(error.response.data, 'validateSigner');
        if (_.isEmpty(error.response.data[signerId])) {
          // If there are no duplicates for this signer, we allow further updates to continue for this signer
          validated = true;
        }
        returnResults = error.response.data;
      }
  ...
  async function populateProspectWithFields(
    ...
    try {
      ...
      if (isEmpty) {
        ...
        const dateOfBirthArray = dateOfBirth !== null
          ? dateOfBirth.split('-')
          : '1977-01-01'.split('-');
        const documentExpirationDateArray = documentExpirationDate !== null
          ? documentExpirationDate.split('-')
          : '1977-01-01'.split('-');
        const documentIssuedDateArray = documentIssuedDate !== null
  ...
  const sendDataToServer = async (
    ...
    try {
      ...
      if (isNewAccountRequest) {
        const create = { institutionId, productConfigurations: prospect.products };
        const referredById = sessionStorage.getItem(magicStrings.props.referredById);

        const accountRequestResp = await AccountRequestService
          .createAccountRequest(referredById ? _.merge(create, { referredById }) : create);
        ...
        if (token) {
          ...
          const accountRequestUpdateResp = await AccountRequestService.updateAccountRequest(
            token,
            accountRequestId,
            {
              status: 'INCOMPLETE',
        ...
        if (signerId && securityToken) {
          const token = (prospect.tokens || []).find((el) => el.id === signerId)?.token!
            || securityToken;
      ...
      dispatch({ type: 'error', error: SignerService.errorMessage(error) });
    }
  };
