...
describe('UrlContext session storage actions', () => {
  ...
  test('should save name of invited person and token to session Storage', async () => {
    const route = '/onboarding/some-prospect-id/signers/some-signerId?name=Ricardo&token=123';
    renderWithReactRouter(
      <UrlProvider>
        <TestingApp />
      </UrlProvider>,
...
describe('UrlContext invitee Token', () => {
  ...
  test('should sanitize token', async () => {
    const route = '/onboarding/some-prospect-id/signers/some-signerId?name=Ricardo&token=javascript:alert("hey user!")';
    renderWithReactRouter(
      <UrlProvider>
        <TestingApp />
      </UrlProvider>,
  ...
  test('should be null if no token in url', async () => {
    const route = '/onboarding/some-prospect-id/signers/some-signerId?name=Ricardo';
    renderWithReactRouter(
      <UrlProvider>
        <TestingApp />
      </UrlProvider>,
...
describe('UrlContext isInvitedByName', () => {
  ...
  test('should sanitize the name', async () => {
    const route = '/onboarding/some-prospect-id/signers/some-signerId?name=javascript:alert("hey user!")';
    renderWithReactRouter(
      <UrlProvider>
        <TestingApp />
      </UrlProvider>,
  ...
  test('should be null if no name in url', async () => {
    const route = '/onboarding/some-prospect-id/signers/some-signerId?token=123';
    renderWithReactRouter(
      <UrlProvider>
        <TestingApp />
      </UrlProvider>,
