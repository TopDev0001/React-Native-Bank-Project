...
const StepSuccess: React.FC<IStepSuccess> = (props) => {
  ...
  const { prospect, sendDataToServer, resetProspect } = useContext(
    ProspectContext
  );
    ...
    return (
      ...
      >
        <div role="alert" className="alert toast is-error" data-testid="error">
          ...
          possible to continue. We&apos;ve notified our engineers of the problem. In
          the meantime you can go back to the form and try again.
        </div>
        <Button
  ...
  return (
    <StepSuccessView props={props} handleRestart={handleRestart} institution={institution} />
  );
};
