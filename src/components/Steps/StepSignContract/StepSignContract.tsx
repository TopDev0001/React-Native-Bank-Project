...
const StepSignContract: FC<IStep> = (props) => {
  ...
  const { prospect, finishProspect, setError } = React.useContext(
    ProspectContext
  );
  ...
  const goToEmbedDocument = async () => {
    ...
    try {
      ...
      const token = (prospect.tokens || []).find((el) => el.id === prospect.signerId)
  ...
  return (
    <StepSignContractView props={props} prospect={prospect} handleSign={handleSign} />
  );
};
