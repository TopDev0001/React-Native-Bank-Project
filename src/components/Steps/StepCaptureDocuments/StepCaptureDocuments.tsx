...
const StepCaptureDocuments: FC<IStepCaptureDocuments> = () => {
  ...
  const [
    ...
    setUserSelectedDocumentType,
  ] = React.useState('');
  const { status, error, documents } = useDocumentState();
  const dispatch = useDocumentDispatch();
  const browser = DetectOS.getBrowser();
  const shouldLoadSdk = !(browser.name === 'Safari' && parseInt(browser.version, 10) <= 13)
  ...
  const handleResults = (result: any) => {
    ...
    let frontDocUrl: string;

    setUserSelectedDocumentType(result.documentType);

    frontDocUrl = typeof result.fullDocumentFrontImage === 'string'
    ...
    if (result.documentType === 'USDL') {
      backDocUrl = typeof result.fullDocumentBackImage === 'string'
  ...
  const handleDocumentTypeChange = (
    ...
    log.info('StepCaptureDocuments', `set document type to ${event.currentTarget.id}`);
  };
    ...
    return (
      <NoCameraMessage title={message}>
        <p>
          ...
          <b>Allow</b>
          {' '}
          on the tooltip prompted by your
          browser.
        </p>
        <p>
          If you don&apos;t have a camera on this device scan this QR code with your
          smartphone or tablet to complete the process.
        </p>
      </NoCameraMessage>
    );
  }
