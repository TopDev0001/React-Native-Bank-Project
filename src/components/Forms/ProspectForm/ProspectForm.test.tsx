...
describe('ProspectForm', () => {
  ...
  test("Continue button should display 'sign and finish' by default", async () => {
    const mockOnClick = jest.fn();
    const { getByTestId, getByText } = renderWithProps({ onSubmit: mockOnClick });

    await waitFor(() => {
      expect(getByText(/Sign and Finish/)).toEqual(getByTestId('step-info-continue'));
    });
  });
  ...
  test('should warn about invalid inputs', async () => {
    ...
    const invalidText = 'sdDQni0TytmnxITwMgJv3uhvDxFoOAIFeVewVmgjtY5MMo0dc2xRt7fv2s5goEvcuBPXPxXZ26Qx' +
  ...
  test('should warn about invalid first name and last name text', async () => {
    ...
    const invalidText = 'sdDQni0TytmnxITwMgJv3uhvDxFoOAIFeVewVmgjtY5MMo0dc2xRt7fv2s5goEv' +
