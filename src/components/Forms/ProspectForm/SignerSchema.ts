...
const schema = yup.object().shape({
  ...
  firstName: yup
    ...
    .test('firstName', 'Invalid First Name', (value) => (!!(value && value.trim().length >= 2))),
  middleName: yup
    .string()
  ...
  lastName: yup
    ...
    .test('lastName', 'Invalid Last Name', (value) => (!!(value && value.trim().length >= 2))),
  dateOfBirth: yup
    .date()
    .nullable()
    .default(null)
    ...
    .test('dateOfBirth', 'You must be older than 18', (value) => (value !== null
      ? moment().diff(moment(value), 'years') >= 18
      : false))
    // tests if we applied a default date
    .test('dateOfBirth', 'Invalid Birth Date', (value) => (moment(value).format('YYYY-MM-DD') !== '1900-01-01')),
  email: yup
    .string()
  ...
  city: yup
    ...
    .test('city', 'Invalid City', (value) => (!!(value && value.trim().length >= 3))),
  ...
  employer: yup
    ...
    .test('employer', 'Invalid Employer', (value) => (!!(value && value.trim().length >= 2))),
});
