...
import { mockInstitution, invalidResponse } from '../../services/__mocks__/Institution';
import { successCreate as mockPhotos, errorCreate as photoError } from '../../services/__mocks__/Photos';
import {
...
const handlers = [
  ...
  rest.post(`${apiURL}/signers/:signerId/email-verifications`, async (req, res, ctx) => {
    if (req.params.signerId === '00000000-9999-aaaa-0000-2ea08a01e903' || req.params.signerId === 'undefined') {
  ...
  rest.put(`${apiURL}/signers/:signerId/email-verifications/:verificationId?token=:token`, async (req, res, ctx) => {
  ...
  rest.get(`${apiURL}/institutions/:id/products`, async (req, res, ctx) => res(ctx.json(listProducts))),

  rest.post(`${apiURL}/documents`, async (req, res, ctx) => {
    const { content, institutionId } = req.body;

    // simulate error conditions
    if (content === 'error' || institutionId === 'error' || institutionId === null) {
      return res(ctx.status(412), ctx.json(photoError));
    }

    return res(ctx.json(mockPhotos));
  }),
  rest.get(`${apiURL}/prospects/identities/:frontDocumentId/:backDocumentId`, async (req, res, ctx) => res(ctx.json(identityInformationSuccess))),
  ...
  // Create Signer
  rest.post(`${apiURL}/signers`, async (req, res, ctx) => res(ctx.set('x-nimble-token', 'sometoken'), ctx.json(postToCreateProspectSuccess))),

  // Create Signer
  rest.put(`${apiURL}/signers/:signerId`, async (req, res, ctx) => res(ctx.json(putToUpdateProspectSuccess))),
  ...
  rest.post(`${apiURL}/signers/:signerId/invites`, async (req, res, ctx) => res(ctx.json('something'))),
  // ACCOUNT Request
  ...
  // Upsert BSA
  rest.post(`${apiURL}/account-requests/:id/bsa-risk-results`, async (req, res, ctx) => res(ctx.json(bsaSucessResponse))),
  ...
  // Get Account Request
  rest.get(`${apiURL}/account-requests/:id`, async (req, res, ctx) => res(ctx.json(getSuccessResponse))),
];
