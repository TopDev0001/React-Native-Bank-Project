...
const FormatHelper = {
  ...
  /* Output to 'MM/DD/YYYY' string */
  dateFormat: (date: Date, divider = '/') => moment(date).format(`MM${divider}DD${divider}YYYY`),
  ...
  stringToCamelCase: (str: string) => str.replace(/(?:^\w|[A-Z]|\b\w|\s+)/g, (match, index) => {
    if (+match === 0) {
      return '';
    }
  ...
  parseDateString: (_value: Date, originalValue: string | Date) => (originalValue === ''
    ? null : moment(originalValue).toDate()
  ),
};
